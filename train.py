...
# dictionary with name of video as key and jump cut timings in seconds as
# values in form of list of tuples. Each tuple is interval in which jump cut has taken place
CUT_DICT = {"v1":[132, 163, 206, 264, 638, 676, 722, 837, 881, 943, 1027, 1089, 1127, 1183,
                  1259, 1336, 1422, 1467, 1504, 1535, 1554, 1599, 1652],
            "v2":[27, 86, 142, 196, 241, 276, 327, 356],
            "v3":[135, 482, 870, 1238, 1631, 2045],
            "v4":[85, 101, 128, 165, 189, 201, 224, 237, 269],
            "v5":[93, 141, 217, 278, 347, 423, 495, 586, 655, 757, 822, 874, 905, 947, 989, 1046,
                  1110, 1145, 1194, 1224, 1269, 1306, 1348, 1378, 1419, ],
            "v6":[263]}
    ...
    for video in CUT_DICT:
        ...
        for cut in CUT_DICT[video]:
            ...
            for frame in req_frame:
                ...
                o, l = object_detection.get_object(frame_loc+frame)
                for i in range(1):
                    try:
                        if o[i] == "person":
                            obj_loc.append(l[i])
            ...
            # x.append(hist_diff)
            if len(obj_loc) == 2:
                X.append([hist_diff[0], abs(obj_loc[0][0]-obj_loc[1][0]),
                          abs(obj_loc[0][1]-obj_loc[1][1])])
            else:
